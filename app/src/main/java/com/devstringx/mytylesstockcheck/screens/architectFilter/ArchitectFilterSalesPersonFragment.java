...
import com.devstringx.mytylesstockcheck.databinding.FragmentArchitectFilterSalesPersonBinding;
...
public class ArchitectFilterSalesPersonFragment extends Fragment implements ResponseListner, NameListWithCheckboxAdapter.FilterCBSelected {
    FragmentArchitectFilterSalesPersonBinding binding;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_architect_filter_sales_person, container, false);
    ...
    private void setupAdapter() {
        RecyclerView recyclerView = binding.architectSalePersonRv;
        adapter = new NameListWithCheckboxAdapter(getContext(), allOwnersList, this::onItemSelectedListner, Common.architectSelectedSaleperson);
    ...
    public void onItemSelectedListner(List<String> selected_items) {
        for (int i = 0; i < allOwnersList.size(); i++) {
            if (allOwnersList.get(i).isSelected()) {
                if (!Common.architectSelectedSalepersonId.contains(allOwnersList.get(i).getId())) {
                    Common.architectSelectedSalepersonId.add(allOwnersList.get(i).getId());
                }
            }else Common.architectSelectedSalepersonId.remove(allOwnersList.get(i).getId());
        }
        for (int i = 0; i < Common.architectSelectedSalepersonId.size(); i++) {
            Common.showLog(Common.architectSelectedSalepersonId.get(i));
        }
        Common.architectSelectedSaleperson = selected_items;
